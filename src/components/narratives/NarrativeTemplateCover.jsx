...
const ImageBlock = ({ src, alt }) => (
  <div className="ntc-img-block">
    <img src={src} alt={alt || "image"} />
  </div>
);

const VideoBlock = ({ src }) => (
  <div className="ntc-video-block">
...
const renderDescriptionBlock = (block, index) => {
  if (typeof block === "string") {
    return <MarkdownBlock key={index} content={block} />;
  }
  ...
  switch (block.type) {
    ...
    case "vimeo":
      return <VimeoBlock key={index} src={block.src} title={block.title} />;
    default:
      return null;
  }
};
  ...
  return (
    ...
    >
      <div className="ntc-container">
        ...
        {/* Título */}
        <h2 className="ntc-title">{NARRATIVE_DATA.title}</h2>

        {/* CONTENIDO */}
        {NARRATIVE_DATA.description.map((block, index) =>
